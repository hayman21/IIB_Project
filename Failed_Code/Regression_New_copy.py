...
for i in range(num_iter):
    ...
    log_alpha = log_likelihood(new_sub.reshape(-1,1), theta_star.reshape(-1,1)) - log_likelihood(initial_sub.reshape(-1,1), GaussianProcess(initial_sub.reshape(-1,1), l, num_obs).reshape(-1,1))
...
for k in range(2):
    ax[1].plot(X, samples[-k], label='Estimate')
